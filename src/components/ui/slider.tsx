...
>(({ className, showTooltip = false, tooltipContent, ...props }, ref) => {
  ...
  React.useEffect(() => {
    if (props.value !== undefined) setInternalValue(props.value as number[])
  }, [props.value])
  ...
  const handlePointerDown = () => { if (showTooltip) setShowTooltipState(true) }
  ...
  const renderThumb = (value: number) => {
    const thumb = (
      <SliderPrimitive.Thumb
        className="block h-5 w-5 rounded-full border-2 border-primary bg-background shadow transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-ring/40 data-[disabled]:cursor-not-allowed"
