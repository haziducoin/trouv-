...
export default function AdminUserPage() {
  ...
  const act = async (action: string, value?: Record<string, unknown>) => {
    ...
    try {
      ...
      else if (r.link) setMagicLink(r.link as string)
      else if (r.message) { setToast(r.message as string); setTimeout(() => setToast(null), 4000) }
      else await load()
    } finally {
      setBusy(null)
    }
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fc]">
          ...
          <div className="flex gap-7">
            ...
            <aside className="w-72 shrink-0">
              ...
              <div className="mt-4 rounded-2xl border border-slate-200/60 bg-white p-4 shadow-sm space-y-2">
                ...
                <button onClick={() => act('reset_password')} disabled={!!busy}
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-700 py-2.5 text-xs font-bold text-white hover:bg-slate-800 disabled:opacity-50">
                  {busy === 'reset_password' ? <Loader2 size={11} className="animate-spin" /> : <Mail size={12} />}
                  Envoyer reset mot de passe
                </button>
                <button onClick={() => act('magic_link')} disabled={!!busy}
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 py-2.5 text-xs font-bold text-white hover:bg-violet-700 disabled:opacity-50">
                  {busy === 'magic_link' ? <Loader2 size={11} className="animate-spin" /> : <KeyRound size={12} />}
                  Générer magic link
                </button>
      ...
      {/* ── Toast succès ── */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 rounded-2xl bg-slate-800 px-5 py-3 shadow-xl">
          ...
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-800">Magic link généré</h3>
              </div>
              <button onClick={() => setMagicLink(null)} className="rounded-xl p-1.5 hover:bg-slate-100">
                <X size={16} className="text-slate-400" />
              </button>
            </div>
            <p className="text-xs text-slate-500 mb-3">Lien à usage unique — valable 1 heure. Envoie-le directement à l'utilisateur.</p>
            <div className="flex items-center gap-2 rounded-xl border border-violet-200 bg-violet-50 p-3">
              <p className="flex-1 truncate text-xs font-mono text-violet-700">{magicLink}</p>
              <button
                onClick={() => { void navigator.clipboard.writeText(magicLink); setToast('Lien copié !'); setTimeout(() => setToast(null), 2000) }}
